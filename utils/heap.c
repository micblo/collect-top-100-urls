...
void heap_insert(heap *h, void* key, void* value) {
    ...
    while (current_index > 0) {
        ...
        if (cmp_func(value, parent->value) < 0) {
    ...
    if (h->active_entries > 0) {
        ...
        while (left_child_index = LEFT_CHILD(current_index), left_child_index < entries) {
            ...
            if (left_child_index+1 < entries) {
                ...
                if (cmp_func(left_child->value, right_child->value) <= 0) {

                    // Swap with the left if it is smaller
                    if (cmp_func(current->value, left_child->value) == 1) {
                ...
                // Right child is smaller
                    ...
                    if (cmp_func(current->value, right_child->value) == 1) {
            ...
            } else if (cmp_func(current->value, left_child->value) == 1) {
                SWAP_ENTRIES(current,left_child);
                current_index = left_child_index;
                current = left_child;
...
void heap_foreach(heap* h, void (*func)(void*,void*,void*), void *ctx) {
